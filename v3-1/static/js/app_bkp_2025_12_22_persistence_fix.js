...
function showToast(message, type = 'info', duration = 5000) {
    ...
    // Error messages stay until user clicks - no auto-dismiss
    if (type !== 'error') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tabBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (currentTabContent && (currentTabContent.id === 'tab-music' || currentTabContent.id === 'tab-form')) {
                ...
                if (musicVal && !musicDownloadTriggered) {
    ...
    if (btnPreviewButtons && previewModal) {
        btnPreviewButtons.addEventListener('click', () => {
            ...
            // Extra
            const extraUrl = document.getElementById('input-extra').value.trim();
            if (extraUrl) {
                const name = document.getElementById('input-extra-name').value.trim() || 'Link Extra';
                const icon = document.getElementById('input-extra-icon').value.trim() || 'fa-solid fa-star';
                buttons.push({ icon, label: name, link: extraUrl, isManual: false });
            }
...
Qualidade 2K.`;
    ...
    async function generateManualAutomatically() {
        ...
        statusEl.textContent = '⏳ Gerando manual formatado...';
        ...
        try {
            const response = await fetch('/api/generate-manual', {
                ...
                body: JSON.stringify({
                    manualText: manualText,
                    threadId: manualThreadId  // Send existing thread ID for context
                })
            });
            const result = await response.json();
            ...
            // Update thread ID for next call (context persistence)
            if (result.threadId) {
                manualThreadId = result.threadId;
                localStorage.setItem('manualThreadId', manualThreadId);
            }
    ...
    // Debounce for manual generation
    let manualTimeout;
    document.getElementById('input-manual').addEventListener('input', () => {
        const statusEl = document.getElementById('manual-status');
        statusEl.textContent = '✍️ Digitando...';
        statusEl.classList.remove('hidden', 'text-green-400', 'text-red-400');
        statusEl.classList.add('text-gray-400');

        manualTimeout = setTimeout(generateManualAutomatically, 2000);
    });
    ...
    if (inputManualFinal) {
        inputManualFinal.addEventListener('input', () => {
            ...
            localStorage.setItem('processedManualText', processedManualText);
            // Optional: Debounce save? For now save immediately or debounce
            saveMediaUrls();
        });
    }
    ...
    async function generateZipPackage(download = true) {
        // Build menuConfig
        const menuConfig = [];
        // Only add manual if processed text exists
        if (processedManualText) {
        ...
        // Extra
        const extraLink = document.getElementById('input-extra').value;
        if (extraLink && extraLink.trim() !== "") {
            const extraName = document.getElementById('input-extra-name').value || "Link Extra";
            const extraIcon = document.getElementById('input-extra-icon').value || "fa-solid fa-star";
            menuConfig.push({ titulo: extraName, icone: extraIcon, link: extraLink });
        }
        ...
        const formData = {
            ...
            extraLink: extraLink,
            extraName: document.getElementById('input-extra-name').value,
            extraIcon: document.getElementById('input-extra-icon').value,
            ...
            slug: document.getElementById('input-slug') ? document.getElementById('input-slug').value : '' // Save Slug
        };
    ...
    function renderHistoryCard(convite) {
        ...
        card.className = 'bg-gray-800 rounded-lg border border-gray-700 overflow-hidden hover:border-purple-500 transition-all group relative max-w-[140px]';
        ...
        // Delete button - delete latest version
        card.querySelector('.delete-history-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            if (latestVersion) showDeleteConfirmation(latestVersion.filename, convite.displayName);
        });
        ...
        popup.innerHTML = `
            <div class="bg-gray-800 rounded-xl p-6 max-w-sm mx-4 border border-gray-700 shadow-2xl">
                <div class="text-center mb-4">
                    ...
                    <p class="text-gray-400">Tem certeza que deseja excluir <strong class="text-white">"${displayName}"</strong>?</p>
...
// --- GitHub Deployment Logic (Outside DOMContentLoaded if needed, or inside) ---
// Since the elements are static in index.html, we can attach inside or outside if we use delegation.
// But let's attach safely inside a check or after DOM load. `DOMContentLoaded` is closed at line 2202.
// Let's create a separate init function or just run it if element exists.
        ...
        btnDeployGithub.addEventListener('click', async () => {
            ...
            try {
                ...
                try {
                    ...
                    throw new Error(`Server Deploy Error (Non-JSON): ${text.substring(0, 100)}...`);
                }

                if (!response.ok && !result.error) { // If error in json, we handle it below
                    throw new Error(`Server Deploy Failed: ${response.status} ${response.statusText}`);
                }
                ...
                deployResult.innerHTML = `
                    <div class="flex flex-col gap-2">
                        <div class="text-green-300 font-bold flex items-center gap-2">
                            <i class="fa-solid fa-check-circle"></i> Sucesso!
                        </div>
                        <a href="${result.live_url}" target="_blank" class="bg-green-600/20 border border-green-500/50 p-3 rounded-lg text-green-300 hover:bg-green-600/30 transition-colors flex items-center justify-between group">
                            <span class="font-mono text-sm truncate">${result.live_url}</span>
                            <i class="fa-solid fa-external-link-alt group-hover:scale-110 transition-transform"></i>
                        </a>
                        <a href="${result.repo_url}" target="_blank" class="bg-gray-700/50 border border-gray-600 p-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-700 transition-colors flex items-center gap-2 text-xs">
                            <i class="fa-brands fa-github text-lg"></i>
                            <span>Abrir Pasta do Convite (Arquivos)</span>
                        </a>
                        <p class="text-[10px] text-green-400/60 mt-1 text-center">(Pode levar alguns minutos para o DNS propagar)</p>
                    </div>
                `;
                deployResult.classList.remove('hidden', 'bg-red-900/30', 'border-red-700', 'text-red-300');
